...
struct gchan { 
  ...
  //  GtkWidget *vol_text; /* VOL text over the knob */
	...
	GtkWidget *httpwind, *httpentry; /* mah */
	int rem;
	unsigned int playmode; /* 0: PLAY 1: LOOP 2: CONT */
	unsigned int channel; /* 1: seekable 2: stream */
	unsigned int idx; /* the channel! */
	/* FIXME: remove it */
	unsigned int pos; /* position in table */ // DEPRECATED
};
...
void gcb_set_channel(GtkWidget *, gint, gint,GdkEventButton *, struct gchan *);
