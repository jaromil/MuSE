...
void PlaylistManager::initPlaylists() {
	...
	for(i=1;i<=plNum;i++) {
		XmlTag *playlistTag=xml->getRootElement(i);
		if(playlistTag) {
...
bool PlaylistManager::save(char *name,Playlist *playlist) {
	...
	if(xml->addRootElement(playlistTag)==XML_NOERR && xml->update()==XML_NOERR) {
...
bool PlaylistManager::update(char *name) {
}

bool PlaylistManager::update(int index) {
}
...
bool PlaylistManagerEntry::update() {
}