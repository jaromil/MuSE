		...
		unsigned int idx, GtkWidget *container)
{
	...
	func("list_add:: item %p item->pos = %u item->container= %p item->data=%p", 
			item, item->pos,
...
void list_debug(GList *list)
{
	...
	notice("DEBUGGING THOUGH list %p", list);
	while(listrunner) {
		tmp= (struct listitem *) listrunner->data;
		if(!tmp)
...
void pack_refresh(GList *list, GtkWidget *table, bool chanlist)
{
	...
	if((tmp=list_find_bypos(list, pos+1))) {
		g_object_ref(G_OBJECT(tmp->container));
		...
		g_object_unref(G_OBJECT(tmp->container));
